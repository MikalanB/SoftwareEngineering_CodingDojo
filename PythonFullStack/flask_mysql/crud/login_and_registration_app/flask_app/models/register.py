...
class User:
    ...
    def validate_register(user):
        ...
        if len(user['first_name']) < 2:
            flash("First name must be at least 2 alphabetic characters", "firstname")
            is_valid = False
        if len(user['last_name']) < 2:
            flash("Last name must be at least 2 alphabetic characters", "lastname")
            is_valid = False
        if not EMAIL_REGEX.match(user['email']):
            flash("This is an invalid email address!",'email')
            is_valid = False
